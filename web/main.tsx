...
import { webApi, startWebReminderEngine, getApiUrl, setApiUrl, isReachable, startConnectionWatch, checkConnectionNow } from './api'
...
import { loadCache } from '../src/offline'
...
function ConnectionGate({ children }: { children: React.ReactNode }): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      void (async () => {
        if (await isReachable()) setReachable(true)
      })()
    }, 10000)
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => void checkConnectionNow(), 1000)
  ...
  // Backend down but a local cache exists (installed clients) — show the app
  // offline; the connection watch re-probes and syncs when the server returns.
  return <>{children}</>
}
